import Link from "next/link";
import { Pencil, Plus } from "lucide-react";

import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { DeleteResourceButton } from "@/components/admin/delete-resource-button";
import { adminFetch, type AdminBlogPost } from "@/lib/admin-api";

export default async function AdminBlogPage() {
  const posts = (await adminFetch<AdminBlogPost[]>("/blog")) ?? [];

  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Blog</h1>
          <p className="mt-1 text-sm text-muted-foreground">Artikel yang tampil di halaman /blog.</p>
        </div>
        <Button render={<Link href="/admin/blog/baru" />}>
          <Plus className="mr-2 h-4 w-4" /> Tulis Artikel
        </Button>
      </div>

      {posts.length === 0 ? (
        <p className="glass mt-8 rounded-2xl p-8 text-center text-muted-foreground">
          Belum ada artikel.
        </p>
      ) : (
        <div className="mt-8 space-y-3">
          {posts.map((post) => (
            <div key={post.id} className="glass flex items-center justify-between gap-4 rounded-2xl p-5">
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="font-medium">{post.title}</h3>
                  <Badge variant={post.is_published ? "secondary" : "outline"} className="text-[10px]">
                    {post.is_published ? "Tayang" : "Draft"}
                  </Badge>
                </div>
                <p className="mt-1 text-sm text-muted-foreground">{post.excerpt}</p>
              </div>
              <div className="flex shrink-0 items-center gap-1">
                <Button
                  render={<Link href={`/admin/blog/${post.id}`} />}
                  size="icon-sm"
                  variant="ghost"
                  aria-label="Edit artikel"
                >
                  <Pencil className="h-4 w-4" />
                </Button>
                <DeleteResourceButton resourcePath={`blog/${post.id}`} />
              </div>
            </div>
          ))}
        </div>
      )}
    </div>
  );
}
